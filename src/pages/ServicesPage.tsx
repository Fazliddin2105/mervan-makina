...
export const ServicesPage: React.FC = () => {
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 py-8 font-sans space-y-10">
      ...
      <PageBanner
        eyebrow="TO'LIQ TEXNIK XIZMAT"
        title="Texnik xizmat, ta'mirlash va YTXK xizmatlari"
        subtitle="Ko'chma servis avtomobillari va Yillik Texnik Xizmat Ko'rsatish (YTXK) shartnomalari yordamida pol yuvish, supurish mashinalari va yuqori bosimli yuvish apparatlaringizni 100% ish holatida saqlang."
      />
        ...
        {SERVICES_DATA.map((svc) => (
          <div key={svc.id} className="surface p-6 sm:p-8 space-y-4 flex flex-col justify-between">
            <div className="space-y-3">
              ...
              <h3 className="text-xl font-extrabold text-[#0B1D3F]">{svc.title}</h3>
              <p className="text-xs text-slate-600 leading-relaxed">{svc.fullDetails}</p>
            ...
            <div className="pt-4 border-t border-slate-100 flex items-center justify-between gap-4">
              ...
              <a href={`tel:${siteSettings.phone.replace(/\s/g, '')}`} className="text-xs text-blue-700 font-bold hover:underline flex items-center gap-1">
                <Phone size={14} /> To'g'ridan-to'g'ri aloqa
              </a>
            </div>
          </div>
        ))}
